...
void StartLight1(void *argument)
{
  ...
  for(;;)
  {
	  osSemaphoreAcquire(BinarySemHandle, osWaitForever); 	// waits forever for semaphore to become available
	  ...
	  osSemaphoreRelease(BinarySemaHandle);		//releases semaphore for next task
  }
  /* USER CODE END 5 */
}
